...
- **자율 서버 구동(Daemon-like Check)**:
  사용자가 수동으로 API 백엔드를 먼저 구동하고 클라이언트를 실행해야 하는 불편함을 없애기 위해, `run_cli.bat`을 실행하면 내부적으로 포트 `8600`의 바인딩 상태를 즉각 감지하고, 꺼져 있는 경우 `run_server.bat`을 새로운 백그라운드 프로세스로 자동 실행한 뒤 5초간의 유예시간을 두어 클라이언트와 연동하는 메커니즘을 자동 구현했습니다.
